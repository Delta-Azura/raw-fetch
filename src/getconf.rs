...
pub fn getconf() -> Result<(Vec<(String, String, String)>)> {
    ...
    if rawconf.contains("mode=binary") {
        let source = rawconf.lines().find(|l| l.starts_with("source=")).context("Failed to get source= line")?.split_once("source=").map(|(_, source)| source).context("Failed to get source path")?.split_whitespace().next().context("")?;
        let index = String::new();
        if source.ends_with("/") {
            let source = source.trim_end_matches("/");
            let index = format!("{}/index.raw", source);
        } else {
            let index = format!("{}/index.raw", source);
        }
        ...
        for i in index.lines() {
            ...
            let release = pkgfile.lines().find(|l| l.contains("release=")).context("Failed to get package release")?.split_once("release=").map(|(_, name)| name).context("Failed to get pkgrel")?.to_string();
